...
import { downloadFileInChunks, getMimeTypeFromFileName, triggerBlobDownload } from './ChunkedDownloader';
      ...
      try {
        ...
        const handleBlobDownload = async (fileInfo, fileName) => {
          try {
            ...
            // Use chunked download approach from the imported utility
            setSuccess(`Downloading ${fileName} using chunked method...`);
            try {
              // Use the imported downloadFileInChunks utility for better reliability
              const blob = await downloadFileInChunks(
                telegramClient,
                fileInfo.id,
                fileInfo.size,
                fileName,
                (progressMessage) => setSuccess(progressMessage)
              );
              
              // Use the imported triggerBlobDownload utility to handle the download
